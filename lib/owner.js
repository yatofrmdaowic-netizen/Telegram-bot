...
const db = new Low(adapter, { users: {}, suspendedUsers: {}, channels: [] });
await db.read();
db.data ||= { users: {}, suspendedUsers: {}, channels: [] };
db.data.users ||= {};
...
🚫 Suspended: *${suspended ? "Yes" : "No"}*`,
  ...
  bot.command("suspendlist", async (ctx) => {
    ...
    return ctx.reply(`🚫 *Suspended Users*\n\n${rows.join("\n")}`, { parse_mode: "Markdown" });
  });
